function convertir_divisas(Q, data, target) {
  ...
  let currencies = {
    ARS: data.last_update_usdt_rate,
    BTC: data.BTC,
    ETH: data.ETH,
  };
...
let selector_precios = {
  ...
  precio_prublicacion: document.getElementsByClassName(
...
const precio_prublicacion = document.getElementsByClassName(
  "andes-money-amount__fraction"
);
const precios_dinamicos = document.getElementsByClassName(
  "dynamic-carousel__price"
);
...
//TAGS
const precios_tag_publicacion = document.getElementsByClassName(
...
function price_update(elements, data, target) {
  ...
  for (let i = 0; i < elements.length; i++) {
    if (elements[i].childNodes.length < 1) {
      ...
      elements[i].innerText = convertir_divisas(precio, data, target).toFixed(
        fixed_length
      );
    } else {
      try {
        ...
        elements[i].innerText =
          "<span>" +
          convertir_divisas(precio, data, target).toFixed(fixed_length) +
          "</span>";
      } catch (e) {
        if (elements[i].innerText !== null) {
          var precio = parseInt(price_cleaner(elements[i].innerText, target));
          elements[i].innerText = convertir_divisas(
            precio,
            data,
            target
          ).toFixed(fixed_length);
        } else {
          var precio = parseInt(
            price_cleaner(
              elements[i].firstChild.childNodes[0].nodeValue,
              target
            )
          );
          elements[i].innerText = convertir_divisas(
      ...
      for (let i = 0; i < data.length; i++) {
        if (data[i].asset_id == "ARS") {
          ...
          chrome.storage.local.set({
            last_update_usdt_rate: cotizacion.toString(),
          });
